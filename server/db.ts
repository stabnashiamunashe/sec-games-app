...
const hasAdminPass = db.prepare('SELECT 1 FROM settings WHERE key = ?').get('admin_password');
if (!hasAdminPass) {
  db.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('admin_password', 'seczimadmin');
}
const hasSimDate = db.prepare('SELECT 1 FROM settings WHERE key = ?').get('simulated_date');
if (!hasSimDate) {
  db.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('simulated_date', '2026-06-30T11:00:00Z');
}
const hasActiveCategory = db.prepare('SELECT 1 FROM settings WHERE key = ?').get('active_category');
if (!hasActiveCategory) {
  db.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('active_category', 'world_cup');
}

// Seed Prediction Teams (SecZim Departments)
const teamCount = db.prepare('SELECT COUNT(*) as count FROM participating_teams').get() as { count: number };
if (teamCount.count === 0) {
  const defaultTeams = [
    {
      id: 'compliance',
      name: 'SecZim Compliance',
  ...
  console.log('Seeded SecZim prediction departments');
}
...
if (gameCount.count === 0) {
  ...
  const r32Kickoffs = [
    '2026-06-28T12:00:00Z', '2026-06-29T16:30:00Z', '2026-06-29T19:00:00Z', '2026-06-29T12:00:00Z',
    '2026-06-30T17:00:00Z', '2026-06-30T12:00:00Z', '2026-06-30T19:00:00Z', '2026-07-01T12:00:00Z',
    '2026-07-01T17:00:00Z', '2026-07-01T13:00:00Z', '2026-07-02T19:00:00Z', '2026-07-02T12:00:00Z',
    '2026-07-02T20:00:00Z', '2026-07-03T18:00:00Z', '2026-07-03T20:30:00Z', '2026-07-03T13:00:00Z'
  ];
  ...
  const r16Kickoffs = [
    '2026-07-04T17:00:00Z', '2026-07-04T12:00:00Z', '2026-07-05T16:00:00Z', '2026-07-05T18:00:00Z',
    '2026-07-06T14:00:00Z', '2026-07-06T17:00:00Z', '2026-07-07T12:00:00Z', '2026-07-07T13:00:00Z'
  ];
  const r16Labels = [
  ...
  const qfKickoffs = ['2026-07-09T16:00:00Z', '2026-07-10T12:00:00Z', '2026-07-11T17:00:00Z', '2026-07-11T20:00:00Z'];
  const qfLabels = [
  ...
  Object.entries(seedApiOutcomes).forEach(([mId, winnerId]) => {
    db.prepare('UPDATE games SET winner_id = ?, finished = "TRUE" WHERE id = ?').run(winnerId, mId);
  });
  ...
  const seczimGames = [
    {
      ...
      stage: 'Compliance Cup Semi 1',
      home_team_id: 'compliance',
      away_team_id: 'supervision',
      home_team_label: 'Compliance Dept',
      away_team_label: 'Supervision Dept',
      home_score: 3,
      away_score: 1,
    ...
    },
    {
      id: 'seczim-2',
      category: 'seczim_games',
      stage: 'Compliance Cup Semi 2',
      home_team_id: 'legal',
      away_team_id: 'finance',
      home_team_label: 'Legal Division',
      away_team_label: 'Finance & Admin',
      home_score: 1,
      away_score: 2,
    ...
    },
    {
      ...
      home_team_label: 'Legal Division',
      away_team_label: 'Executive Team',
      home_score: null,
      away_score: null,
  ...
  seczimGames.forEach(g => {
    ...
    if (g.finished === 'TRUE') {
      db.prepare('UPDATE games SET home_score = ?, away_score = ? WHERE id = ?').run(g.home_score, g.away_score, g.id);
    }
  });
...
// Seed Custom Direct Points (Chess, Card games, Table Tennis, etc. direct awards)
const directPointsCount = db.prepare('SELECT COUNT(*) as count FROM direct_points').get() as { count: number };
if (directPointsCount.count === 0) {
  const initialDirectPoints = [
    { id: 'dp-1', team_id: 'compliance', game_name: 'SecZim Chess Championship (1st Place)', points: 50, date_awarded: '2026-06-25T14:00:00Z' },
    { id: 'dp-2', team_id: 'supervision', game_name: 'SecZim Chess Championship (2nd Place)', points: 30, date_awarded: '2026-06-25T14:30:00Z' },
    { id: 'dp-3', team_id: 'finance', game_name: 'Annual Corporate Card Game (Winner)', points: 40, date_awarded: '2026-06-26T16:00:00Z' },
    { id: 'dp-4', team_id: 'legal', game_name: 'Annual Corporate Card Game (Runner Up)', points: 20, date_awarded: '2026-06-26T16:30:00Z' },
    { id: 'dp-5', team_id: 'executive', game_name: 'SecZim Tug of War friendly', points: 30, date_awarded: '2026-06-27T10:00:00Z' }
  ];

  const insertDirectPoints = db.prepare(`
    INSERT INTO direct_points (id, team_id, game_name, points, date_awarded)
    VALUES (@id, @team_id, @game_name, @points, @date_awarded)
  `);

  initialDirectPoints.forEach(dp => insertDirectPoints.run(dp));
  console.log('Seeded initial direct non-prediction sports points');
}
