...
export default function BracketView({
  ...
  >({});
  const [currentScorePredictions, setCurrentScorePredictions] = useState<
  ...
  const handleChangePasscode = async (e: React.FormEvent) => {
    ...
    if (
      !/^\d{4}$/.test(currentTeamPasscode) ||
      !/^\d{4}$/.test(newTeamPasscode)
    ) {
      setPasscodeChangeMessage(
        "Current and new PINs must be exactly 4 digits.",
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        alert(
          data.message ||
            "Passcode updated successfully. Please sign in again.",
        );
        handleLogoutTeam();
      } else {
  ...
  const isKickoffLocked = (
    kickoff?: string | null,
    referenceTime: Date | string | null = currentTime,
  ): boolean => {
    const kickoffTime = kickoff ? new Date(kickoff).getTime() : null;
    const currentTimeValue =
      referenceTime instanceof Date
        ? referenceTime
        : new Date(referenceTime || 0);
    const referenceTimestamp = currentTimeValue.getTime();

    if (
      kickoffTime === null ||
      Number.isNaN(kickoffTime) ||
      Number.isNaN(referenceTimestamp)
    )
      return false;
    return kickoffTime <= referenceTimestamp;
  };
  ...
  const formatScoreInputValue = (value: number | null | undefined): string => {
  ...
  const handleScorePredictionChange = (
    ...
    const isStarted = isMatchLocked(gameId, kickoff);
    if (isStarted) return;

    const cleaned = value.replace(/\D/g, "").slice(0, 2);
    const scoreValue = cleaned === "" ? null : Number(cleaned);

    setCurrentScorePredictions((prev) => {
      const current = prev[gameId] || { homeScore: null, awayScore: null };
      return {
        ...prev,
        [gameId]: {
          homeScore: side === "home" ? scoreValue : current.homeScore,
          awayScore: side === "away" ? scoreValue : current.awayScore,
        },
      };
    });
  };

  const renderScorePredictionInputs = (
    gameId: string,
    homeLabel: string,
    awayLabel: string,
    kickoff?: string,
  ) => {
    const scorePrediction = currentScorePredictions[gameId] || {
      homeScore: null,
      awayScore: null,
    };
      ...
      <div className="mt-2 border-2 border-slate-200 bg-slate-50 p-2.5 rounded-none">
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2">
          <label className="min-w-0">
            <span className="block text-[9px] font-black uppercase tracking-widest text-brand-dark truncate">
              {homeLabel}
            </span>
            <input
              type="text"
              ...
              maxLength={2}
              disabled={isLocked}
              value={formatScoreInputValue(scorePrediction.homeScore)}
              onChange={(e) =>
          ...
          <label className="min-w-0">
            ...
            <input
              ...
              maxLength={2}
              disabled={isLocked}
              value={formatScoreInputValue(scorePrediction.awayScore)}
              onChange={(e) =>
                handleScorePredictionChange(
                  gameId,
                  "away",
  ...
  const savePredictionsToDb = async (
    ...
    const authPin = prompt(
      `Please re-enter your 4-digit passcode PIN for ${activeTeam.name} to confirm saving:`,
    );
    if (!authPin) return;
    ...
    try {
      const response = await fetch("/api/predictions", {
        ...
        body: JSON.stringify({
          ...
          passcode: authPin,
          predictions: updated,
          scorePredictions: updatedScores,
  ...
  const handleSelectSecZimWinner = (
    ...
    const isStarted = isMatchLocked(gameId, gameKickoff);
    if (isStarted) {
  ...
  const handleQuickFill = () => {
    ...
    visualBracket.R32.forEach((m) => {
      ...
      if (!isStarted) {
        const candidates = [m.homeTeamId, m.awayTeamId].filter(
          Boolean,
        ) as string[];
    ...
    if (!isFinalStarted) {
      const finalCand = [finalMatch.homeTeamId, finalMatch.awayTeamId].filter(
        Boolean,
      ) as string[];
  ...
  const handleReset = () => {
    ...
    const updatedScores = { ...currentScorePredictions };
    const allMatches = [
      ...visualBracket.R32,
      ...visualBracket.R16,
      ...visualBracket.QF,
    ...
    allMatches.forEach((m) => {
      ...
      if (!isStarted) {
        updated[m.id] = "";
        updatedScores[m.id] = { homeScore: null, awayScore: null };
      }
    });
    ...
    games
      ...
      .forEach((g) => {
        const isStarted = isMatchLocked(g.id, g.kickoff);
        if (!isStarted) {
          updated[g.id] = "";
          updatedScores[g.id] = { homeScore: null, awayScore: null };
        }
      });

    setCurrentPredictions(updated);
    setCurrentScorePredictions(updatedScores);
  };

  const getActiveStageMatches = (): Match[] => {
    switch (selectedStage) {
      case "R32":
        return visualBracket.R32;
      case "R16":
        return visualBracket.R16;
      case "QF":
      ...
      <div className="max-w-md mx-auto bg-white border-4 border-brand-dark rounded-none p-6 sm:p-8 space-y-6 text-center shadow-none">
        ...
        <div>
          ...
          <p className="text-xs text-brand-dark-light mt-2 uppercase tracking-wide">
            ...
            <span className="font-bold font-mono text-brand-gold">
              {activeTeam.name.toUpperCase()}
            </span>
          </p>
          <p className="text-xs text-brand-dark-muted mt-2">
            Please enter your team's 4-digit passcode PIN to unlock predictions
            and submit brackets.
          </p>
        </div>
        ...
        <div className="border-t-2 border-brand-dark pt-4 text-left">
          ...
          <select
            ...
            style={{
              borderLeftWidth: "6px",
              borderLeftColor: activeTeam.color,
            }}
          >
    ...
    <div className="space-y-6" id="predictions-bracket-section">
      ...
      <div className="bg-white rounded-none p-4 sm:p-5 border-4 border-brand-dark shadow-none flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-black text-brand-dark text-base uppercase tracking-tight">
              {activeTeam.name} Predicts
            </h3>
      ...
      {showPasscodeForm && (
        <form
          ...
          className="bg-white border-4 border-brand-dark p-4 rounded-none space-y-3"
        >
          <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
            <KeyRound className="w-4 h-4 text-brand-gold" />
            <h4 className="text-xs font-black uppercase tracking-widest text-brand-dark">
              Change Team PIN
            </h4>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input
              ...
              onChange={(e) =>
                setCurrentTeamPasscode(e.target.value.replace(/\D/g, ""))
              }
            ...
            <input
              ...
              onChange={(e) =>
                setConfirmTeamPasscode(e.target.value.replace(/\D/g, ""))
              }
        ...
        <button
          ...
          className={`py-2.5 text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            activeCategory === "world_cup"
              ...
              : "text-brand-dark-slate hover:bg-brand-dark-medium"
          }`}
        >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {getActiveStageMatches().map((match) => {
              ...
              const isLocked = isMatchLocked(match.id, match.kickoff);
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    {renderScorePredictionInputs(
                      match.id,
                      homeTeam?.name || match.homeTeamLabel || "Home",
                      awayTeam?.name || match.awayTeamLabel || "Away",
                      match.kickoff,
                    )}
                  </div>
                </div>
          ...
          {seczimGames.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {seczimGames.map((game) => {
                const homePredictionTeam = getPredictionTeamObj(
                  game.home_team_id,
                );
                const awayPredictionTeam = getPredictionTeamObj(
                  game.away_team_id,
                );
                const currentPrediction = currentPredictions[game.id];
                const isLocked = isMatchLocked(game.id, game.kickoff);
                  ...
                  >
                    ...
                    <div className="bg-brand-dark text-white px-4 py-2 flex justify-between items-center text-xs">
                      <div>
                        ...
                        <span className="font-bold uppercase font-sans tracking-wide">
                          SECZIM CORPORATE EVENT
                        </span>
                      </div>
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundColor: `${homePredictionTeam?.color || "#C09138"}15`,
                          }}
                        >
                      ...
                      >
                        <div
                          className="w-10 h-10 border-2 border-brand-dark flex items-center justify-center text-xl shrink-0"
                          style={{
                            backgroundColor: `${awayPredictionTeam?.color || "#313131"}15`,
                          }}
                        >
